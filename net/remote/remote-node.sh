...
cat >> ~/solana/on-reboot <<EOF
  ...
  (
    sudo PANOPTIS_METRICS_CONFIG="$PANOPTIS_METRICS_CONFIG" scripts/oom-monitor.sh
  ) > oom-monitor.log 2>&1 &
...
EOF
  ...
  bootstrap-validator)
    ...
    if [[ $skipSetup != true ]]; then
      clear_config_dir "$PANOPTIS_CONFIG_DIR"
...
EOF
  ...
  validator|blockstreamer)
    ...
    if [[ $skipSetup != true ]]; then
      ...
      if [[ $nodeType = blockstreamer ]]; then
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/solana/config/blockstreamer-identity.json "$PANOPTIS_CONFIG_DIR"/validator-identity.json
      else
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/solana/config/validator-identity-"$nodeIndex".json "$PANOPTIS_CONFIG_DIR"/validator-identity.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/solana/config/validator-stake-"$nodeIndex".json "$PANOPTIS_CONFIG_DIR"/stake-account.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/solana/config/validator-vote-"$nodeIndex".json "$PANOPTIS_CONFIG_DIR"/vote-account.json
      fi
      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/solana/config/shred-version "$PANOPTIS_CONFIG_DIR"/shred-version

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/solana/config/bank-hash "$PANOPTIS_CONFIG_DIR"/bank-hash || true

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/solana/config/faucet.json "$PANOPTIS_CONFIG_DIR"/faucet.json
    fi
    ...
    args=(
      ...
      --gossip-port 10015
      --rpc-port 8328
      --expected-shred-version "$(cat "$PANOPTIS_CONFIG_DIR"/shred-version)"
    )
    if [[ $nodeType = blockstreamer ]]; then
      args+=(
    ...
    if [[ -r "$PANOPTIS_CONFIG_DIR"/bank-hash ]]; then
      args+=(--expected-bank-hash "$(cat "$PANOPTIS_CONFIG_DIR"/bank-hash)")
    fi
    ...
    scp "$entrypointIp":~/solana/config/faucet.json "$PANOPTIS_CONFIG_DIR"/
    if [[ $nodeType = blockstreamer ]]; then
...
EOF
    ...
    if [[ $skipSetup != true && $nodeType != blockstreamer && -z $maybeSkipAccountsCreation ]]; then
      ...
      if [[ ${extraPrimordialStakes} -eq 0 ]]; then
        ...
        multinode-demo/delegate-stake.sh --vote-account "$PANOPTIS_CONFIG_DIR"/vote-account.json \
                                         --stake-account "$PANOPTIS_CONFIG_DIR"/stake-account.json \
