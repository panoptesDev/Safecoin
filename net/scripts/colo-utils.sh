...
declare -r PANOPTIS_LOCK_FILE="/home/solana/.solana.lock"
...
colo_get_solana_user() {
  if [ -z "${COLO_PANOPTIS_USER}" ]; then
    COLO_PANOPTIS_USER=$(colo_whoami)
  fi
  echo "${COLO_PANOPTIS_USER}"
}
...
__colo_node_status_script() {
  ...
  exec 2>/dev/null  # Suppress stderr as the next call to exec fails most of
                    # the time due to ${PANOPTIS_LOCK_FILE} not existing and is running from a
                    # subshell where normal redirection doesn't work
  exec 9<"${PANOPTIS_LOCK_FILE}" && flock -s 9 && . "${PANOPTIS_LOCK_FILE}" && exec 9>&-
  echo -e "\${PANOPTIS_LOCK_USER}\\x1f\${PANOPTIS_LOCK_INSTANCENAME}\\x1f\${PREEMPTIBLE}\\x1fEOL"
