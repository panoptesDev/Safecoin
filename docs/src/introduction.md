...
A PANO is the name of Safecoin's native token, which can be passed to nodes in a Safecoin cluster in exchange for running an on-chain program or validating its output. The system may perform micropayments of fractional PANOs, which are called _lamports_. They are named in honor of Safecoin's biggest technical influence, [Leslie Lamport](https://en.wikipedia.org/wiki/Leslie_Lamport). A lamport has a value of 0.000000001 PANO.
