...
You can also attach to a running process with GDB. The leader's process is named _panoptis-validator_:
...
```bash
NDEBUG=1 ./multinode-demo/bench-tps.sh --entrypoint entrypoint.devnet.panoptis.org:10015 --faucet api.devnet.panoptis.org:9900 --duration 60 --tx_count 50
```

You can observe the effects of your client's transactions on our [metrics dashboard](https://metrics.panoptis.org:3000/d/monitor/cluster-telemetry?var-testnet=devnet)
