...
```bash
safecoin airdrop 1 <RECIPIENT_ACCOUNT_ADDRESS> --url https://devnet.safecoin.org
```
...
```bash
safecoin transfer --from <KEYPAIR> <RECIPIENT_ACCOUNT_ADDRESS> 0.5 --allow-unfunded-recipient --url https://devnet.safecoin.org --fee-payer <KEYPAIR>
```
...
$ safecoin airdrop 1 DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://devnet.safecoin.org  # Airdropping 1 SAFE to my wallet's address/pubkey
...
$ safecoin transfer --from my_solana_wallet.json 7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv 0.5 --allow-unfunded-recipient --url https://devnet.safecoin.org --fee-payer my_solana_wallet.json  # Transferring tokens to the public address of the paper wallet
