    ...
    let matches = App::new(crate_name!()).about(crate_description!())
        ...
        .arg(
            Arg::with_name("snapshot_archive_format")
                ...
                .alias("snapshot-compression") // Legacy name used by Panoptis v1.5.x and older
        ...
        .arg(
            Arg::with_name("account_indexes")
                ...
                .takes_value(true)
                .multiple(true)
                .possible_values(&["program-id", "pano-token-owner", "pano-token-mint"])
                .value_name("INDEX")
    ...
    let mut validator_config = ValidatorConfig {
        ...
        rpc_addrs: value_t!(matches, "rpc_port", u16).ok().map(|rpc_port| {
            (
                ...
                // https://github.com/panoptisdev/panoptis/issues/12250
            )
        }),
...
fn process_account_indexes(matches: &ArgMatches) -> AccountSecondaryIndexes {
    let account_indexes: HashSet<AccountIndex> = matches
        ...
        .map(|value| match value {
            ...
            "pano-token-mint" => AccountIndex::SafeTokenMint,
            "pano-token-owner" => AccountIndex::SafeTokenOwner,
            _ => unreachable!(),
        })
        .collect();
