...
#
set -e
PANOPTIS_ROOT="$(cd "$(dirname "$0")"/..; pwd)"
...
if [[ ! -f update_manifest_keypair.json ]]; then
  "$PANOPTIS_ROOT"/scripts/panoptis-install-update-manifest-keypair.sh "$OS"
fi
...
PATH="$PANOPTIS_ROOT"/target/debug:$PATH
...
# shellcheck disable=SC2086 # Don't want to double quote $maybeKeypair
panoptis-install deploy $maybeKeypair --url "$URL" "$DOWNLOAD_URL" update_manifest_keypair.json
