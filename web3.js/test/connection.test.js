...
import {Token, u64} from '@solana/pano-token';
...
describe('Connection', () => {
  ...
  it('get confirmed block', async () => {
    ...
    await expect(
      ...
      `Block not available for slot ${Number.MAX_PANO_INTEGER}`,
    );
  });
  ...
  if (process.env.TEST_LIVE) {
    describe('token methods', () => {
      ...
      it('get confirmed token transaction', async () => {
        ...
        if (ix.parsed) {
          expect(ix.program).to.eq('pano-token');
      ...
      it('get parsed token account info', async () => {
        ...
        if (accountInfo) {
          ...
          if (data instanceof Buffer) {
            ...
            expect(data.program).to.eq('pano-token');
  ...
  it('getVersion', async () => {
    await mockRpcResponse({
      method: 'getVersion',
      params: [],
      value: {'panoptis-core': '0.20.4'},
    });

    const version = await connection.getVersion();
    expect(version['panoptis-core']).to.be.ok;
  });
