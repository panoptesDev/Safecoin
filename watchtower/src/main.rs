...
fn get_config() -> Config {
    let matches = App::new(crate_name!())
        ...
        define environment variables before running `panoptis-watchtower`:
        ...
        define environment variable before running `panoptis-watchtower`:
    ...
    loop {
        ...
        if let Some((failure_test_name, failure_error_message)) = &failure {
            ...
            if !last_notification_msg.is_empty() {
                ...
                info!("{}", all_clear_msg);
                notifier.send(&format!("panoptis-watchtower: {}", all_clear_msg));
            }
