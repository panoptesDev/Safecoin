...
(
  ...
  cp "${RELEASE_BASENAME}"/bin/panoptis-install-init panoptis-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml panoptis-install-init-"$TARGET"* $MAYBE_TARBALLS; do
...
PANOPTIS_RELEASE=$CHANNEL_OR_TAG
PANOPTIS_INSTALL_INIT_ARGS=$CHANNEL_OR_TAG
PANOPTIS_DOWNLOAD_ROOT=http://release.solana.com
EOF
  cat install/panoptis-install-init.sh >> release.solana.com-install
