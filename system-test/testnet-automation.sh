...
if [[ -z $PANOPTIS_METRICS_CONFIG ]]; then
  if [[ -z $PANOPTIS_METRICS_PARTIAL_CONFIG ]]; then
    echo PANOPTIS_METRICS_PARTIAL_CONFIG not defined
    exit 1
  fi
  export PANOPTIS_METRICS_CONFIG="db=$TESTNET_TAG,host=$INFLUX_HOST,$PANOPTIS_METRICS_PARTIAL_CONFIG"
fi
echo "PANOPTIS_METRICS_CONFIG: $PANOPTIS_METRICS_CONFIG"
