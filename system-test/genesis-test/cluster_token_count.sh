...
usage: $0 [cluster_rpc_url]
 ...
 Required arguments:
   cluster_rpc_url  - RPC URL and port for a running Panoptis cluster (ex: http://34.83.146.144:8328)
EOF
  exit $exitcode
}

function get_cluster_version {
  clusterVersion="$(curl -s -X POST -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","id":1, "method":"getVersion"}' "$url" | jq '.result | ."panoptis-core" ')"
