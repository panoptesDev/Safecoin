...
```bash
panoptis-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
panoptis-tokens transaction-log --output-path transactions.csv
```
...
```bash
panoptis-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
panoptis-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
panoptis-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
...
```bash
panoptis-tokens pano-token-balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
...
```bash
panoptis-tokens distribute-pano-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
the `--dry-run` parameter, or `panoptis-tokens balances`.
...
```bash
panoptis-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
